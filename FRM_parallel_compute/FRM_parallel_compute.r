...
###################### if there is error try it later ###############################################
i = 1
while ("try-error" %in% class(frm_dd)) {
    Sys.sleep(60 * i)
    i = i + 1
    frm_dd = try(frm_data_download(firm_names, a, b, a_m, b_m))
}
write.csv(format(frm_dd$data, scientific = FALSE), file = paste("100_firms_returns_and_scaled_macro_", 
